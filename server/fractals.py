...
void fractal_kernel(
    ...
    const REAL min_re, const REAL min_im,
    const REAL step_re, const REAL step_im,
    const REAL julia_re, const REAL julia_im,
    const int max_iter,
    const int mode,
    const int palette,
    const int interior,
    const int ssaa)
{
            ...
            for (int sx = 0; sx < 2; sx++) {
                ...
                if (mode == 0) {
                    t = escape_time(0.0, 0.0,
                        min_re + px * step_re, min_im + py * step_im, max_iter);
                } else {
                    t = escape_time(
                        min_re + px * step_re, min_im + py * step_im,
                        julia_re, julia_im, max_iter);
                }
                if (t < 0.0) {
    ...
    if (mode == 0) {
        t = escape_time(0.0, 0.0,
            min_re + (REAL)x * step_re, min_im + (REAL)y * step_im, max_iter);
    } else {
        t = escape_time(
            min_re + (REAL)x * step_re, min_im + (REAL)y * step_im,
            julia_re, julia_im, max_iter);
    }
...
def _launch(kernel: cp.RawKernel, out, view: dict) -> None:
    ...
    # cupy marshals Python floats as float64 regardless of the kernel's declared
    # type, which misaligns the argument buffer for fp32 kernels. Cast explicitly.
    use_double = bool(view.get("precision", 0))
    real_t = np.float64 if use_double else np.float32
    ...
    kernel(
        ...
        (
            ...
            real_t(min_re), real_t(min_im),
            real_t(step), real_t(step),
            real_t(view["juliaRe"]), real_t(view["juliaIm"]),
            np.int32(view["maxIter"]),
            np.int32(view["mode"]),
            np.int32(view["palette"]),
            np.int32(view["interior"]),
            np.int32(view.get("ssaa", 1)),
        ),
    )
...
def warmup() -> None:
    ...
    for use_double in (False, True):
        ...
        k((1, 1), (16, 16), (
            out,
            np.int32(8), np.int32(8),
            real_t(-2.5), real_t(-1.0),
            real_t(0.2), real_t(0.2),
            real_t(-0.8), real_t(0.156),
            np.int32(64),
